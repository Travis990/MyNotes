...
int
copy_header(struct bufferevent *bev, int op, char *msg, char *filetype, long filesize)
{
	char buf[4096]={0};
...
int
copy_file(struct bufferevent *bev, const char *strFile)
{
...
int
send_dir(struct bufferevent *bev, const char *strPath)
{
	// 需要拼出来一个html页面发送给客户端
...
// 处理请求函数
int
http_request(struct bufferevent *bev, char *path)
{
...
// cfd 变化触发函数
void
bevent_cb(struct bufferevent *bev, short what, void *ctx)
{
	if( what & BEV_EVENT_EOF ) // 如果客户端关闭执行
	{
		printf("client closed\n");
		bufferevent_free(bev);
	}
	else if( what & BEV_EVENT_ERROR ) // 发生错误执行
	{
		printf("err to client closed\n");
		bufferevent_free(bev); // 释放监听树节点
	}
		...
		struct sockaddr* addr, int socklen, void *arg)
{
	...
	bufferevent_setcb(bev, read_cb, NULL, bevent_cb, base); // 设置回调:缓冲区读回调,cfd变化回调bevent_cb
...
int main(int argc, char** argv)
{
	...
	struct evconnlistener * listener = evconnlistener_new_bind( // 连接监听器
			base, // 根节点
			listen_cb, // 监听回调函数
		   	(void*)base, // 要监听的上树节点
			LEV_OPT_CLOSE_ON_FREE|LEV_OPT_REUSEABLE, // 关闭时自动释放,端口复用
			-1, // listen函数关键参数自动选用合适值
			(struct sockaddr*)&serv, // 地址绑定信息
			sizeof(serv)
	);
