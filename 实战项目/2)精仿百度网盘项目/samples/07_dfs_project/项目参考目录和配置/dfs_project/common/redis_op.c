 ...
 */
/* -------------------------------------------*/
int rop_zset_exit(redisContext *c, char *key, char *member)
{
    int ret = 0;
    redisReply *reply;
    reply = redisCommand(c,"zrange %s 0 -1", key); //type is status
    if(reply->type !=  REDIS_REPLY_ARRAY)
    {
        LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]rop_zset_exit: %s,member: %s Error:%s,%s\n", key, member,reply->str, c->errstr);
        goto END;
    }
    int i;
