...
int main()
{
	...
	//设置 curl 的属性和操作
	//CURLOPT_URL：设置访问URL
	curl_easy_setopt(curl, CURLOPT_URL, "http://127.0.0.1:8888/login?type=10&user=测试&pwd=123456");
	
	//最后一个参数设置为1，表示启用时会发送一个常规的POST请求
	curl_easy_setopt(curl, CURLOPT_POST, 1);
	
	// 指定post内容 
	curl_easy_setopt(curl, CURLOPT_POSTFIELDS, "this is a post data\n");
